...
#include "stdlib.h"
#include "string.h"
#include "pair_tanhlr_cut_idealc.h"
#include "atom.h"
#include "comm.h"
#include "force.h"
#include "neighbor.h"
...
PairTanhlrCutIdealc::PairTanhlrCutIdealc(LAMMPS *lmp) : Pair(lmp)
{
  respa_enable = 0;
}
...
PairTanhlrCutIdealc::~PairTanhlrCutIdealc()
{
  if (allocated) {
...
void PairTanhlrCutIdealc::compute(int eflag, int vflag)
{
...
void PairTanhlrCutIdealc::allocate()
{
  allocated = 1;
  int n = atom->ntypes;
...
void PairTanhlrCutIdealc::settings(int narg, char **arg)
{
...
void PairTanhlrCutIdealc::coeff(int narg, char **arg)
{
...
double PairTanhlrCutIdealc::init_one(int i, int j)
{
  // printf("idealc, i is %d, j is %d\n", i,j);
  ...
  // printf("idealc,cut is %f\n",cut[i][j]);
  //   printf("idealc,%d\n", atom->ntypes);
  //   printf("idealc,end init_one\n");
  return cut[i][j];
}
...
void PairTanhlrCutIdealc::write_restart(FILE *fp)
{
...
void PairTanhlrCutIdealc::read_restart(FILE *fp)
{
...
void PairTanhlrCutIdealc::write_restart_settings(FILE *fp)
{
...
void PairTanhlrCutIdealc::read_restart_settings(FILE *fp)
{
  int me = comm->me;
  if (me == 0) {
...
double PairTanhlrCutIdealc::single(int i, int j, int itype, int jtype, double rsq,
...
/* ---------------------------------------------------------------------- */
double PairTanhlrCutIdealc::memory_usage()
{
