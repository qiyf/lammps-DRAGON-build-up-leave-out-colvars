...
void PairTanhlrCutIdeal::compute(int eflag, int vflag)
{
  ...
  double xtmp,ytmp,ztmp,delx,dely,delz,evdwl,fpair, scale_factor;
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      scale_factor = 1.0;
      int ctcfL_l, ctcfL_r, ctcfR_l, ctcfR_r, idxL_l, idxL_r, idxR_l, idxR_r;
      if (jglobal > iglobal) {
      ...
      // } else if (ctcfR - ctcfL <= minLoopLen) {
      //     scale_factor = 0.0;
      // } else if (ctcfR - ctcfL > maxLoopLen) {
      //     scale_factor = 0.0;
      // }
      else {
            // only include the ctcf with near neighbour
            for (int ictcf=0;ictcf<nctcfl;ictcf++) {
              if (rawctcfList_l[ictcf] == ctcfL_l){
                idxL_l = ictcf;
              }
              if (rawctcfList_l[ictcf] == ctcfR_l){
                idxR_l = ictcf;
              }
            }

            for (int ictcf=0;ictcf<nctcfr;ictcf++) {
              if (rawctcfList_r[ictcf] == ctcfL_r){
                idxL_r = ictcf;
              }
              if (rawctcfList_r[ictcf] == ctcfR_r){
                idxR_r = ictcf;
              }
            }

            if (((idxR_l - idxL_l) > ctcf_neigh) || ((idxR_r - idxL_r) > ctcf_neigh)) {
              // scale_factor = 0.0;
              continue;
            }
...
// the second argument is the input file for ideal chromosome potential
  ...
  if (narg != 9) error->all(FLERR,"Illegal pair_style ideal command");
  ...
  // minimum loop size
  // minLoopLen = atoi(arg[4]);
  // maxLoopLen = atoi(arg[5]);

  // pass through the raw ctcf position
  ...
  if (fp == NULL)
    error->all(FLERR,"Cannot open the ctcf id file");
  memory->create(rawctcfList_l,nctcfl, "pair:rawctcfList");
  idx = 0;
  while(fgets(line,102400,fp)) {
    ptr = strtok(line," \t\n\r\f");
    rawctcfList_l[idx] = atoi(ptr);
    idx++;
  }
  fclose(fp);
  ...
  if (fp == NULL)
    error->all(FLERR,"Cannot open the ctcf id file");
  memory->create(rawctcfList_r,nctcfr, "pair:rawctcfList");
  idx = 0;
  while(fgets(line,102400,fp)) {
    ptr = strtok(line," \t\n\r\f");
    rawctcfList_r[idx] = atoi(ptr);
    idx++;
  }
  fclose(fp); 

  ctcf_neigh = atoi(arg[8]);
