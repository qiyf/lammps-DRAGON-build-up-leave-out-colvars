...
void PairTanhlrCutIdeal::compute(int eflag, int vflag)
{
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      // only <--2 1--> cannot form
      if ( (type[i] == 1) && (type[j] == 2) && (iglobal > jglobal) ) continue;
      if ( (type[j] == 1) && (type[i] == 2) && (jglobal > iglobal) ) continue;
      ...
      if ( (rsq < cutsq[itype][jtype]) && (jmi<nideal) ){
        ...
        //} else if ( ( (type[i]==1) && (type[j]==2) ) || ( (type[i]==2) && (type[j]==1) ) || ( (type[i]==1) && (type[j]==4) ) || ( (type[i]==4) && (type[j]==1) ) || ( (type[i]==2) && (type[j]==4) ) || ( (type[i]==4) && (type[j]==2) || ( (type[i]==4) && (type[j]==4) ) ) ) {
...
// the second argument is the input file for ideal chromosome potential
  ...
  if (narg != 5) error->all(FLERR,"Illegal pair_style ideal command");
