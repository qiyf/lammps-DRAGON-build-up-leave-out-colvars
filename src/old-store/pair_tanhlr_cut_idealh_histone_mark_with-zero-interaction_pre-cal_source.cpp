  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if ( (rsq < cutsq[itype][jtype]) && (jmi<nideal) ){
        ...
        //printf("atom %d, atom %d, first ideal, %f \t", iglobal,jglobal, ideal_alpha);

        hm_flag1 = active[iglobal-1]; // vector for histone mark 1
        hm_flag2 = active[jglobal-1]; // vector for histone mark 2
        
        for (int ihm=0;ihm<nHistoneMark;ihm++){
          for (int jhm=0;jhm<nHistoneMark;jhm++){
            hm_flag1 = active[iglobal-1][ihm];
            hm_flag2 = active[jglobal-1][jhm];
            // printf("%d, %d\n", hm_flag1,hm_flag2);
            if (hm_flag1==0 && hm_flag2==0){
                ideal_alpha += hm_potential1[jmi]; //hm_potential1 stands for 0-0
            } else if (hm_flag1==1 && hm_flag2==1){
                ideal_alpha += hm_potential3[jmi][ihm][jhm]; //hm_potential1 stands for 1-1
            } else {
                if (hm_flag1==1){
                  if (hm_flag2!=0) error->all(FLERR,"error in histone index2");
                    ideal_alpha += hm_potential2[jmi][ihm];
              } else if (hm_flag2==1){
                  if (hm_flag1!=0) error->all(FLERR,"error in histone index1");
                    ideal_alpha += hm_potential2[jmi][jhm];
              } else{
...
// the second argument is the input file for ideal chromosome potential
  ...
  // pass through the hm potential file 0-0, 2d
  fp = fopen(arg[4],"r");
  
  if (fp == NULL)
    error->all(FLERR,"Cannot open the hm1 potential file");

  memory->create(hm_potential1,nideal, "pair:hm");
  ...
  while(fgets(line,102400,fp)) {
    ...
    ptr = strtok(NULL," \t\n\r\f");
    if (!ptr) error->all(FLERR,"Incorrectly formatted hm 1 chromosome potential file");
    hm_potential1[idx] = force->numeric(FLERR, ptr) * 0.5;
  }
  fclose(fp); 


  // pass through the hm potential file 0-1, 2d
  fp = fopen(arg[5],"r");
  
  if (fp == NULL)
    error->all(FLERR,"Cannot open the hm2 potential file");

  memory->create(hm_potential2,nideal, nHistoneMark, "pair:hm");
  ...
  while(fgets(line,102400,fp)) {
    ...
    for (int ihm=0; ihm<nHistoneMark; ihm++) {
          ...
          hm_potential2[idx][ihm] = force->numeric(FLERR, ptr) * 0.5;   // note that 0.5 is added here. This is because the contact function
  ...
  // pass through the hm potential file 1-1, 3d
  fp = fopen(arg[6],"r");
  
  if (fp == NULL)
    error->all(FLERR,"Cannot open the hm3 potential file");

  memory->create(hm_potential3,nideal, nHistoneMark, nHistoneMark, "pair:hm");
  ...
  while(fgets(line,102400,fp)) {
    ...
    for (int ihm=0; ihm<nHistoneMark; ihm++) {
        for (int jhm=ihm; jhm<nHistoneMark; jhm++) {
            ...
            if (!ptr) error->all(FLERR,"Incorrectly formatted hm chromosome potential file");
            hm_potential3[idx][ihm][jhm] = force->numeric(FLERR, ptr) * 0.5;   // note that 0.5 is added here. This is because the contact function
                                                                                  // has 0.5. It is been added here to the ideal potential so that
                                                                                  // there is no need to do this for force calculation. If look 
                                                                                  // at function init_one, this is also why ptanh is half of htanh

            hm_potential3[idx][jhm][ihm] = hm_potential3[idx][ihm][jhm];
        }
    }
  }
  fclose(fp);  

  // pass through the active id file, histone marks here
  fp = fopen(arg[7],"r");
  if (fp == NULL)
                         ...
                         double &fforce)
{
  ...
  if ( (imol==jmol) && (jmi<nideal) ) {
    ...
    for (int ihm=0;ihm<nHistoneMark;ihm++){
      for (int jhm=0;jhm<nHistoneMark;jhm++){
        hm_flag1 = active[jglobal-1][jhm];
        hm_flag2 = active[iglobal-1][ihm];
        if (hm_flag1==0 && hm_flag2==0){
          ideal_alpha += hm_potential1[jmi]; //hm_potential1 stands for 0-0
        } else if (hm_flag1==1 && hm_flag2==1){
          ideal_alpha += hm_potential3[jmi][ihm][jhm]; //hm_potential1 stands for 1-1
