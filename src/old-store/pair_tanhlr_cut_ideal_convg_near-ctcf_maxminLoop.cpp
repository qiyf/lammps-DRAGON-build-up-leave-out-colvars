...
void PairTanhlrCutIdeal::compute(int eflag, int vflag)
{
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      scale_factor = 1.0;
      int ctcfL, ctcfR, dL, dR, idxL, idxR;
      if (jglobal > iglobal) {
      ...
      if ( (ctcfL==-1) || (ctcfR==-1) ) {
          ...
          if ( dL>10 && dR>10 ) {
            ...
            for (int ictcf=0;ictcf<nctcf;ictcf++) {
              if (rawctcfList[ictcf] == ctcfL){
                idxL = ictcf;
              }
              if (rawctcfList[ictcf] == ctcfR){
                idxR = ictcf;
              }
            }

            if ((idxR - idxL) > ctcf_neigh){
              scale_factor = 0.0;
            }
...
// the second argument is the input file for ideal chromosome potential
  ...
  if (fp == NULL)
    error->all(FLERR,"Cannot open the ctcf id file");
  memory->create(rawctcfList,nctcf, "pair:rawctcfList");
  idx = 0;
  while(fgets(line,102400,fp)) {
    ptr = strtok(line," \t\n\r\f");
    rawctcfList[idx] = atoi(ptr);
    idx++;
  }
  fclose(fp); 
