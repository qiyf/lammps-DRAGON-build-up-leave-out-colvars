...
ComputeStyle(pressure/local,ComputePressureLocal)

#else

#ifndef LMP_COMPUTE_PRESSURE_LOCAL_H
#define LMP_COMPUTE_PRESSURE_LOCAL_H

#include "compute.h"

namespace LAMMPS_NS {

class ComputePressureLocal : public Compute {
 public:
  ComputePressureLocal(class LAMMPS *, int, char **);
  ~ComputePressureLocal();
  void init();
 ...
 private:
  ...
  class KSpace *kspace;
  double origin[3], radii, radii_sq;

  void pair_contribution();
  double compute_invol_fraction(double *, double *, double);
};
