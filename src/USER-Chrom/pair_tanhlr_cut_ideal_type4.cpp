  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if ( (ctcfL==-1) || (ctcfR==-1) ) {
          ...
          //printf("ever here i? %d, %d, %d, %d, %d, %12.6f \n", iglobal, jglobal, ctcfL, ctcfR, ctcfR-ctcfL, scale_factor);
          //double dl = (double) ctcfR-ctcfL;
          //scale_factor = exp(-dl/200.0);
      }
      ...
      if ( (rsq < cutsq[itype][jtype]) && (jmi<nideal) ){
        ...
        if ( (type[i]==3) && (type[j]==3) ) {
            ...
            // no need to check i global vs jglobal; already used continue to break out
...
// the second argument is the input file for ideal chromosome potential
  ...
  double decay_length;
  decay_length = force->numeric(FLERR,arg[3]);
