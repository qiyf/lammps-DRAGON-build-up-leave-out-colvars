  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if ( (rsq < cutsq[itype][jtype]) && (jmi<nideal) ){
        ...
        // printf("%.3f\n", ideal_alpha);
        if ((iglobal >= 1) && (iglobal <= 14) && (jglobal >= 1) && (jglobal <= 14)) ideal_alpha *= fac; //{printf("yes\n"); ideal_alpha *= fac; printf("%.3f\n", ideal_alpha);}
        else if ((iglobal >= 15) && (iglobal <= 28) && (jglobal >= 15) && (jglobal <= 28)) ideal_alpha *= fac;
//        if ((iglobal >= 496) && (iglobal <= 505) && (jglobal >= 496) && (jglobal <= 505)) ideal_alpha *= fac; //{printf("yes\n"); ideal_alpha *= fac; printf("%.3f\n", ideal_alpha);}
//        else printf("%.3f\t%d\t%d\n", ideal_alpha, iglobal, jglobal);
      	else ideal_alpha = -0.04;
        ...
        // printf("%.3f\t%.3f\t%.3f\t%.3f\n",rexp,tanr,ideal_alpha,utanh);
        fpair = factor_lj/r*ideal_alpha*10.0*(1-tanr*tanr);
        ...
        if (eflag) {
          evdwl = utanh - offset[0][0] * ideal_alpha;
          evdwl *= factor_lj;
        }
...
// the second argument is the input file for ideal chromosome potential
  ...
  if (fp == NULL)
    error->all(FLERR,"Cannot open the alpha file");
  ...
  while(fgets(line,102400,fp)) {
    ...
    ideal_potential[idx] = force->numeric(FLERR, ptr);
    //printf("%.3f\n", ideal_potential[idx]);
  }
  fclose(fp);

  fac = force->numeric(FLERR,arg[3]);
...
double PairTanhlrCutIdeald::init_one(int i, int j)
{
  ...
  // printf("ideald,cut is %f\n",cut[i][j]);
  //   printf("ideald,%d\n", atom->ntypes);
  //   printf("ideald,end init_one\n");
  return cut[i][j];
}
