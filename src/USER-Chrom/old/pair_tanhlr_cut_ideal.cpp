  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if ( (type[i] == 3) && (type[j] == 3) ) {
          /*
          //if ( (ctcfList[iglobal-1][0] != ctcfList[jglobal-1][0]) && (ctcfList[iglobal-1][1] != ctcfList[jglobal-1][1]) ) continue;
          int dleft, dright;
          dleft  = abs(ctcfList[iglobal-1][0] - ctcfList[jglobal-1][0]);
          dright = abs(ctcfList[iglobal-1][1] - ctcfList[jglobal-1][1]);
          scale_factor = pow(0.5,dleft) * pow(0.5,dright);
          //if (ctcfList[iglobal-1][1] != ctcfList[jglobal-1][1]) continue;
          */
          continue;
      }
...
// the second argument is the input file for ideal chromosome potential
  ...
  memory->create(ideal_potential,nideal,2,"pair:ideal");
                         ...
                         double &fforce)
{
  ...
  scale_factor = 1.0;
  if ( (itype == 3) && (jtype == 3) ) {
      //if ( (ctcfList[iglobal-1][0] != ctcfList[jglobal-1][0]) && (ctcfList[iglobal-1][1] != ctcfList[jglobal-1][1]) ) return 0.0 ;
      /*
      int dleft, dright;
      dleft  = abs(ctcfList[iglobal-1][0] - ctcfList[jglobal-1][0]);
      dright = abs(ctcfList[iglobal-1][1] - ctcfList[jglobal-1][1]);
