...
#include "stdlib.h"
#include "string.h"
#include "pair_tanhlr_cut_compt.h"
#include "atom.h"
#include "comm.h"
#include "force.h"
#include "neighbor.h"
...
PairTanhlrCutCompartment::PairTanhlrCutCompartment(LAMMPS *lmp) : Pair(lmp)
{
  respa_enable = 0;
}
...
PairTanhlrCutCompartment::~PairTanhlrCutCompartment()
{
  if (allocated) {
...
void PairTanhlrCutCompartment::compute(int eflag, int vflag)
{
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      iglobal = atom->tag[i];
      jglobal = atom->tag[j];
      itype   = domain[iglobal-1];
      jtype   = domain[jglobal-1];
...
void PairTanhlrCutCompartment::allocate()
{
  allocated = 1;
  int n = atom->ntypes;
...
void PairTanhlrCutCompartment::settings(int narg, char **arg)
{
...
void PairTanhlrCutCompartment::coeff(int narg, char **arg)
{
...
double PairTanhlrCutCompartment::init_one(int i, int j)
{
...
void PairTanhlrCutCompartment::write_restart(FILE *fp)
{
...
void PairTanhlrCutCompartment::read_restart(FILE *fp)
{
...
void PairTanhlrCutCompartment::write_restart_settings(FILE *fp)
{
...
void PairTanhlrCutCompartment::read_restart_settings(FILE *fp)
{
  int me = comm->me;
  if (me == 0) {
...
double PairTanhlrCutCompartment::single(int i, int j, int itype, int jtype, double rsq,
                         ...
                         double &fforce)
{
  ...
  int iglobal, jglobal, icomp, jcomp;

  iglobal = atom->tag[i];
  jglobal = atom->tag[j];
  icomp   = domain[iglobal-1];
  jcomp   = domain[jglobal-1];

  if (r <= rmh[icomp][jcomp]) {
      rexp = (rmh[icomp][jcomp]-r)*sigmah[icomp][jcomp];
      tanr = tanh(rexp);
      utanh = ptanh[icomp][jcomp]*(1.0 + tanr);

      fforce = factor_lj/r*ptanh[icomp][jcomp]*(1.0-tanr*tanr)*sigmah[icomp][jcomp];
  } else {
      utanh = ptanh[icomp][jcomp]*rmh4[icomp][jcomp] / rsq / rsq;
      // there is an extra r here which is to normalize position vector
      fforce = factor_lj/r*ptanh[icomp][jcomp]*rmh4[icomp][jcomp]* (4.0) /rsq/rsq/r;
  }

  phitanh = utanh - offset[icomp][jcomp];
  return factor_lj*phitanh;
}

/* ---------------------------------------------------------------------- */
double PairTanhlrCutCompartment::memory_usage()
{
