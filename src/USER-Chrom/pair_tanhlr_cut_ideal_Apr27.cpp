  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if ( (type[i] == 3) && (type[j] == 3) ) {
          ...
          int rounded = (int) ceil( (dleft+dright)/2.0 );
          scale_factor = scale_matrix[rounded];
      }
...
// the second argument is the input file for ideal chromosome potential
  ...
  memory->create(scale_matrix,5000, "pair:ctcfList");
  scale_matrix[0] = 1.0;
  scale_matrix[1] = 0.7;
  scale_matrix[2] = 0.7;
  scale_matrix[3] = 0.5;
  scale_matrix[4] = 0.5;
  for (idx=5; idx<5000; idx++) 
    //scale_matrix[idx] = pow(0.7,idx); 
    scale_matrix[idx] = 0.2; 
