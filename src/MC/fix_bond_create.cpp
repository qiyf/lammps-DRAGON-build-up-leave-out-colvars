  ...
  created(NULL), copy(NULL), random(NULL)
{
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg],"iparam") == 0) {
      ...
      iarg += 2;
    // added by Bin
    } else if (strcmp(arg[iarg],"convergent") == 0) {
      if (iarg+2 > narg) error->all(FLERR,"Illegal fix bond/create command");
      convergentFlag = force->inumeric(FLERR,arg[iarg+1]);
      iarg += 2;
...
{
  int i,j,k,m,n,ii,jj,inum,jnum,itype,jtype,n1,n2,n3,possible,iglobal,jglobal;
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (itype == iatomtype && jtype == jatomtype) {
        ...
        // iatomtype == 7: ctcf+
        // jatomtype == 19: ctcf-
        // for convergent orientation, iglobal must be less than jglobal
        if ( (convergentFlag==1) && (iglobal>jglobal) ) 
            possible = 0;
      } else if (itype == jatomtype && jtype == iatomtype) {
        if ((jmaxbond == 0 || bondcount[i] < jmaxbond) &&
            (imaxbond == 0 || bondcount[j] < imaxbond))
          possible = 1;
        if ( (convergentFlag==1) && (jglobal>iglobal) ) 
            possible = 0;
      }
