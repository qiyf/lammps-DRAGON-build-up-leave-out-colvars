...
/*
 * added by Bin on 4-10-2018
 * Here we would like to choose the reference atoms from part of the system as well, 
 * instead of from a reference file. 
 * The challenge is to add the force for reference atoms as well.
 */
colvar::orientation_flexRef::orientation_flexRef(std::string const &conf)
  : cvc(conf)
{
  function_type = "orientation_flexRef";
  atoms = parse_group(conf, "atoms");
  atoms_ref = parse_group(conf, "atoms_ref");

  //ref_pos.reserve(atoms_ref->size());

  // implemented in the colvartypes.h
  // initialize rot member data
  if (!atoms->noforce) {
    rot.request_group2_gradients(atoms->size());
  }

  if (!atoms_ref->noforce) {
    rot.request_group1_gradients(atoms_ref->size());
  }
...
colvar::orientation_flexRef::orientation_flexRef()
  : cvc()
{
  function_type = "orientation_flexRef";
  x.type(colvarvalue::type_quaternion);
...
void colvar::orientation_flexRef::calc_value()
{
  rot.b_debug_gradients = is_enabled(f_cvc_debug_gradient);

  rot.calc_optimal_rotation(atoms_ref->positions(), atoms->positions());
...
void colvar::orientation_flexRef::calc_gradients()
{
  // gradients have already been calculated and stored within the
  // member object "rot"; we're not using the "grad" member of each
  // atom object, because it only can represent the gradient of a
...
void colvar::orientation_flexRef::apply_force(colvarvalue const &force)
{
  cvm::quaternion const &FQ = force.quaternion_value;
  ...
  // apply force to the reference atoms
  if (!atoms_ref->noforce) {
    for (size_t ia = 0; ia < atoms_ref->size(); ia++) {
      for (size_t i = 0; i < 4; i++) {
        (*atoms_ref)[ia].apply_force(FQ[i] * rot.dQ0_1[ia][i]);
      }
    }
  }
}
