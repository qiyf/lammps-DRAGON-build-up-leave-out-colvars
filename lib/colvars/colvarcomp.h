...
/// \brief Colvar component: q for protein folding
...
protected:
  ...
  // 
  void allocate();
  cvm::real Sigma(int sep);

private:
  bool allocated;

  int n;
  bool qbias_flag, qbias_exp_flag, qobias_flag, qobias_exp_flag, cutoff_flag;
  cvm::real sigma, sigma_exp;
  cvm::real cutoff, min_sep;
  cvm::real *sigma_sq;
  int l;
  cvm::real **rN, **r, **q;
...
public:
  /// Constructor
  qbias (std::string const &conf);
  qbias();
  //virtual inline ~qbias() {}
  virtual ~qbias();
  virtual void calc_value();
  virtual void calc_gradients();
  virtual void calc_force_invgrads();
...
public:
  /// Constructor
  qinterface (std::string const &conf);
  qinterface();
  //virtual inline ~qbias() {}
  virtual ~qinterface();
  virtual void calc_value();
...
/// \brief Colvar component: Debye-Huckel Electrostatics
/// (colvarvalue::type_scalar type, range (-\infty:\infty))
class colvar::dhenergy
  : public colvar::cvc
{
protected:
  /// Atoms involved
...
private:
  ...
  bool allocated;
  int n_group1, n_group2;
  cvm::real cutoff, I, T, epsilon;
  cvm::real k, constant;
  cvm::real *charge1, *charge2;

public:
  /// Constructor
  dhenergy (std::string const &conf);
  dhenergy();
  //virtual inline ~dhenergy() {}
  virtual ~dhenergy();
  virtual void calc_value();
  virtual void calc_gradients();
  virtual void calc_force_invgrads();
...
/// \brief Colvar component: orientation in space of an atom group,
/// with respect to a set of reference coordinates that is dynamically moving as well.
/// added by Bin; 4-10-2018
/// (colvarvalue::type_quaternion type, range
/// [-1:1]x[-1:1]x[-1:1]x[-1:1])
class colvar::orientation_flexRef
  : public colvar::cvc
{
protected:
...
public:
  ...
  orientation_flexRef(std::string const &conf);
  orientation_flexRef();
  virtual inline ~orientation_flexRef() {}
  virtual void calc_value();
  virtual void calc_gradients();
  virtual void apply_force(colvarvalue const &force);
  virtual cvm::real dist2(colvarvalue const &x1,
...
// added by Bin
simple_scalar_dist_functions(qbias)
simple_scalar_dist_functions(qinterface)
simple_scalar_dist_functions(dhenergy)
...
inline cvm::real colvar::orientation_flexRef::dist2(colvarvalue const &x1,
                                            colvarvalue const &x2) const
{
  return x1.quaternion_value.dist2(x2);
}

inline colvarvalue colvar::orientation_flexRef::dist2_lgrad(colvarvalue const &x1,
                                                    colvarvalue const &x2) const
{
  return x1.quaternion_value.dist2_grad(x2);
}

inline colvarvalue colvar::orientation_flexRef::dist2_rgrad(colvarvalue const &x1,
                                                    colvarvalue const &x2) const
{
  return x2.quaternion_value.dist2_grad(x1);
}
