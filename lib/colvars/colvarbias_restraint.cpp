...
int colvarbias_restraint_harmonic::init(std::string const &conf)
{
  ...
  get_keyval(conf, "lowerWall", b_lower_flag, false);
  get_keyval(conf, "upperWall", b_upper_flag, false);
...
{
  if ( (b_lower_flag) && (x->value()>xcenter) ) return 0.0;

  if ( (b_upper_flag) && (x->value()<xcenter) ) return 0.0;
...
// gaussian bias
colvarbias_restraint_gaussian::colvarbias_restraint_gaussian(char const *key)
  : colvarbias_restraint(key)
{
    b_lower_flag   = 0;
    b_upper_flag   = 0;
...
int colvarbias_restraint_gaussian::init(std::string const &conf)
{
  colvarbias_restraint::init(conf);
...
cvm::real colvarbias_restraint_gaussian::restraint_potential(cvm::real k,
                                                             colvar const *x,
                                                             colvarvalue const &xcenter) const
{
  if ( (b_lower_flag) && (x->value()>xcenter) ) return 0.0;
  ...
  return - k * exp(- x->dist2(x->value(), xcenter)/4.0); // this constant should be changed
}


colvarvalue colvarbias_restraint_gaussian::restraint_force(cvm::real k,
                                                           colvar const *x,
                                                           colvarvalue const &xcenter) const
{
  if ( (b_lower_flag) && (x->value()>xcenter) ) return 0.0;
  ...
  if ( (b_upper_flag) && (x->value()<xcenter) ) return 0.0;
  return k /4.0 * exp(- x->dist2(x->value(), xcenter)) * x->dist2_lgrad(x->value(), xcenter);// the constant 4 should be varied.
}


cvm::real colvarbias_restraint_gaussian::restraint_convert_k(cvm::real k,
                                                             cvm::real dist_measure) const
{
  return k / (dist_measure * dist_measure);
}
