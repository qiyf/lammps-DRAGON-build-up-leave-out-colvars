...
void PairTanhlrCutIdeala::compute(int eflag, int vflag)
{
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      iglobal = atom->tag[i];
      jglobal = atom->tag[j];
      jmi = abs(floor(jglobal/cg_factor)-floor(iglobal/cg_factor));

      // ideal potential is only applicable to the same molecule.
      if ( (rsq < cutsq[itype][jtype]) && (imol==jmol) && (jmi<nideal) && (jmi>0) ){
        r = sqrt(rsq);

        double ideal_alpha;
        if ( (active[jglobal]==1) && (active[iglobal]==1) ) {
            ideal_alpha = ideal_potential[jmi][0];
        } else if ( (active[jglobal]==2) && (active[iglobal]==2) ) {
...
// the second argument is the input file for ideal chromosome potential
  ...
  // coarse graining
  cg_factor = force->numeric(FLERR,arg[3]);
  ...
  for (idx=0; idx<nideal; ++idx) {
    printf("ideal potential: ? %d %f %f %f\n", idx, ideal_potential[idx][0], ideal_potential[idx][1], ideal_potential[idx][2]);
  }
  */
  //for (idx=0; idx<ntotal; ++idx) {
    //printf("ideal potential: ? %d %d\n", idx, active[idx]);
  //}
