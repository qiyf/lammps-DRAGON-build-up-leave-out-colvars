...
#include "stdlib.h"
#include "string.h"
#include "pair_tanhlr_cut_ctcf.h"
#include "atom.h"
#include "comm.h"
#include "force.h"
#include "neighbor.h"
...
PairTanhlrCutCtcf::PairTanhlrCutCtcf(LAMMPS *lmp) : Pair(lmp)
{
  respa_enable = 0;
}
...
PairTanhlrCutCtcf::~PairTanhlrCutCtcf()
{
  if (allocated) {
    ...
    memory->destroy(ctcfId);
  }
}

/* ---------------------------------------------------------------------- */

void PairTanhlrCutCtcf::compute(int eflag, int vflag)
{
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      jmi = abs(domain[jglobal-1] - domain[iglobal-1]);

      // ideal potential is only applicable to the same molecule.
      //if ( (rsq < cutsq[itype][jtype]) && (imol==jmol) && (ctcfId[jglobal-1] != -1) && (ctcfId[iglobal-1] != -1) && (cti<nideal) && (jmi>8) ){
      if ( (rsq < cutsq[itype][jtype]) && (imol==jmol) && (ctcfId[jglobal-1] != -1) && (ctcfId[iglobal-1] != -1) && (jmi<nideal)){
        r = sqrt(rsq);
        
        //printf("ctcf: %d %d %d %d %d %d", iglobal, jglobal, ctcfId[iglobal-1], ctcfId[jglobal-1], domain[iglobal-1], domain[jglobal-1]);
        double ideal_alpha;
...
void PairTanhlrCutCtcf::allocate()
{
  allocated = 1;
  int n = atom->ntypes;
...
void PairTanhlrCutCtcf::settings(int narg, char **arg)
{
...
// the second argument is the input file for ideal chromosome potential
  ...
  char line[1024];
  if (fp == NULL)
    error->all(FLERR,"Cannot open the ctcf potential file");
  ...
  while(fgets(line,1024,fp)) {
    ...
    // The second site
    if (!ptr)
      error->all(FLERR,"Incorrectly formatted ctcf potential file");
  ...
  memory->create(ctcfId,ntotal,"pair:ctcf");
...
void PairTanhlrCutCtcf::coeff(int narg, char **arg)
{
...
double PairTanhlrCutCtcf::init_one(int i, int j)
{
...
void PairTanhlrCutCtcf::write_restart(FILE *fp)
{
...
void PairTanhlrCutCtcf::read_restart(FILE *fp)
{
...
void PairTanhlrCutCtcf::write_restart_settings(FILE *fp)
{
...
void PairTanhlrCutCtcf::read_restart_settings(FILE *fp)
{
  int me = comm->me;
  if (me == 0) {
...
double PairTanhlrCutCtcf::single(int i, int j, int itype, int jtype, double rsq,
                         ...
                         double &fforce)
{
  ...
  if ( (imol==jmol) && (ctcfId[jglobal-1] != -1) && (ctcfId[iglobal-1] != -1) && (jmi<nideal) ){
    ideal_alpha = ideal_potential[jmi];
...
/* ---------------------------------------------------------------------- */
double PairTanhlrCutCtcf::memory_usage()
{
