  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsq[itype][jtype]) {
        ...
        double ptanh_gamma;
        //ptanh_gamma = ptanh[itype][jtype] * 0.5*(1+tanh(compt_sigma*(jmi-compt_rc)));
        ptanh_gamma = ptanh[itype][jtype] * 0.5*(1+tanh(compt_sigma*(compt_rc-jmi)));
...
void PairTanhlrCutCompartment::allocate()
{
  ...
  memory->create(rmh4,n+1,n+1,"pair:rmh");
...
void PairTanhlrCutCompartment::settings(int narg, char **arg)
{
  ...
  compt_sigma = force->numeric(FLERR,arg[1]);
  compt_rc    = force->numeric(FLERR,arg[2]);
                         ...
                         double &fforce)
{
  ...
  double ptanh_gamma;
  //ptanh_gamma = ptanh[itype][jtype] * 0.5*(1.0+tanh(compt_sigma*(jmi-compt_rc)));
  ptanh_gamma = ptanh[itype][jtype] * 0.5*(1.0+tanh(compt_sigma*(compt_rc-jmi)));
