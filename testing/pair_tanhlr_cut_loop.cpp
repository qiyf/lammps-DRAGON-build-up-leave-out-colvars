...
#include "stdlib.h"
#include "string.h"
#include "pair_tanhlr_cut_loop.h"
#include "atom.h"
#include "comm.h"
#include "force.h"
#include "neighbor.h"
...
PairTanhlrCutLoop::PairTanhlrCutLoop(LAMMPS *lmp) : Pair(lmp)
{
  respa_enable = 0;
}
...
PairTanhlrCutLoop::~PairTanhlrCutLoop()
{
  if (allocated) {
...
void PairTanhlrCutLoop::compute(int eflag, int vflag)
{
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if ( (rsq < cutsq[itype][jtype]) && (jmi<=MaxLoop) ) {
        ...
        double ptanh_gamma;
        ptanh_gamma = ptanh[itype][jtype] * pow(jmi, (g_a-1.0)) * exp(-jmi/g_b) / g_norm;
...
void PairTanhlrCutLoop::allocate()
{
  allocated = 1;
  int n = atom->ntypes;
...
void PairTanhlrCutLoop::settings(int narg, char **arg)
{
...
void PairTanhlrCutLoop::coeff(int narg, char **arg)
{
...
double PairTanhlrCutLoop::init_one(int i, int j)
{
...
void PairTanhlrCutLoop::write_restart(FILE *fp)
{
...
void PairTanhlrCutLoop::read_restart(FILE *fp)
{
...
void PairTanhlrCutLoop::write_restart_settings(FILE *fp)
{
...
void PairTanhlrCutLoop::read_restart_settings(FILE *fp)
{
  int me = comm->me;
  if (me == 0) {
...
double PairTanhlrCutLoop::single(int i, int j, int itype, int jtype, double rsq,
...
/* ---------------------------------------------------------------------- */
double PairTanhlrCutLoop::memory_usage()
{
